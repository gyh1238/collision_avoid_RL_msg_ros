...
from stage_world1_test import StageWorld
...
def run(comm, env, policy, policy_path, action_bound, optimizer):
    ...
    for id in range(MAX_EPISODES):
        ...
        while not terminal and not rospy.is_shutdown():
            ...
            # execute actions
#	    print('scaled_action.shape: ', scaled_action.shape)
	    scaled_action = np.squeeze(scaled_action)
#	    print('scaled_action.shape: ', scaled_action.shape)
...
#                    n_agent = NUM_ENV
#                    s_batch = s_batch.reshape(-1, n_agent, 3, 512)
#                    goal_batch = goal_batch.reshape(-1, n_agent, 2)
#                    speed_batch = speed_batch.reshape(-1, n_agent, 2)
#                    a_batch = a_batch.reshape(-1, n_agent, 2)
#                    r_batch = r_batch.reshape(-1, n_agent, 1)
#                    d_batch = d_batch.reshape(-1, n_agent, 1)
#                    l_batch = l_batch.reshape(-1, n_agent, 1)
#                    v_batch = v_batch.reshape(-1, n_agent, 3, 512)
#                    t_batch = t_batch.reshape(-1, n_agent, 3, 512)
#                    advs_batch = _batch.reshape(-1, n_agent, 6)
...
if __name__ == '__main__':
    ...
    if rank == 0:
        ...
        policy = CNNPolicy(msg_action_space=6, ctr_action_space=2, frames=LASER_HIST, n_agent=NUM_ENV)
        policy.cuda()
